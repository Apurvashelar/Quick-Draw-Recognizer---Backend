...
class Net(nn.Module):
    def __init__(self):  # Constructor
        ...
        self.conv1 = nn.Conv2d(1, 32, 5, 4)
        self.conv2 = nn.Conv2d(32, 64, 5, 4)
        self.dropout1 = nn.Dropout(0.25)
        self.dropout2 = nn.Dropout(0.5)
        self.fc1 = nn.Linear(7744,40)
...
def train(model, use_cuda, train_loader, optimizer, epoch):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):  # Get the batch
        ...
        # exit(0) # Forward pass through the model
...
def main():
    ...
    dataset1 = customDatasetClass(path=r'C:\Users\apurv\Downloads\GoogleDataImages_train\GoogleDataImages_train')
    
      # Get the train dataset
    dataset2 = customDatasetClass(path=r'C:\Users\apurv\Downloads\GoogleDataImages_test\GoogleDataImages_test') 

    # Get the train dataloader
    train_loader = torch.utils.data.DataLoader(dataset1, num_workers=2, shuffle=True, batch_size=16)
    # Get the test dataloader
    test_loader = torch.utils.data.DataLoader(dataset2, num_workers=2, shuffle=False, batch_size=16)
